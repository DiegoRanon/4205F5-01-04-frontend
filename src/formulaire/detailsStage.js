...
const StageDetails = () => {
  ...
  const checkingPostuler = async function () {
    ...
    try {
      ...
      reponseData = await sendRequest(`https://frontend-qhl0.onrender.com/stage/getEtudiants/${stage.id}`);
      if (reponseData && reponseData.etudiants) {
        const studentDetails = reponseData.etudiants.map(etudiantId =>
          sendRequest(`https://frontend-qhl0.onrender.com/etudiant/${etudiantId}`)
        );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        ...
        reponseData = await sendRequest(
            `https://frontend-qhl0.onrender.com/stage/postulerStage/${stage.id}`,
            "POST",
            JSON.stringify({
